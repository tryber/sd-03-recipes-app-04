...
function CockTailsControlsRecipeProgress(props) {
  ...
  function handleChecked(event, value, type) {
    ...
    // changeChecked(checked, countChecked, type, id);
  }
  return (
    <div>
      ...
      {data.map((element, i) => (
        <div key={element.meal} data-testid={`${i}-ingredient-step`}>
          <span>
            <input id={i} type="checkbox" checked={checkLocalStorage.cocktails[id][i].checked} name={element.meal} onClick={(event) => handleChecked(event, checked.checkbox[i].checked, 'cocktails')} />
